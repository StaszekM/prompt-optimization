...
set +x
set -a
source "$HOME/.config/sbatch-secrets/secrets.env"
set +a
...
srun --label --chdir=$WORKSPACE_ROOT uv venv --python $python_version
echo [INFO]: Installing packages from repo on nodes
srun --label --chdir=$WORKSPACE_ROOT uv sync
echo [INFO]: Installing vllm in the virtual environment on nodes
srun --label --chdir=$WORKSPACE_ROOT uv pip install --quiet --python $python_version vllm==0.26.0 "tokenizers>=0.22.0,<0.23" ninja

export VIRTUAL_ENV="$WORKSPACE_ROOT/.venv"
export PATH="$VIRTUAL_ENV/bin:$PATH"

setsid env CUDA_VISIBLE_DEVICES=0 "$VIRTUAL_ENV/bin/vllm" serve $GENERATOR_MODEL_PATH --data-parallel-size 1  --served-model-name $GENERATOR_MODEL_NAME --port 8001 > ~/prompt-optimization/vllm-model-generator.log 2>&1 &
PID_A=$!

setsid env CUDA_VISIBLE_DEVICES=1 "$VIRTUAL_ENV/bin/vllm" serve $REFLECTION_MODEL_PATH --data-parallel-size 1 --served-model-name $REFLECTION_MODEL_NAME --port 8002 > ~/prompt-optimization/vllm-model-reflection.log 2>&1 &
PID_B=$!
